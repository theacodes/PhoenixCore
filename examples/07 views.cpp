...
int main()
{
    ...
    */
    ...
    while( system.run() )
    {
        ...
        */
        if( EventReceiver::getKey( PHK_LEFT ) ) system.getView().setPosition( system.getView().getPosition() + ( Vector2d( -100.0f , 0.0f ) * timer.getTime() ) );
        if( EventReceiver::getKey( PHK_RIGHT ) ) system.getView().setPosition( system.getView().getPosition() + ( Vector2d( 100.0f , 0.0f ) * timer.getTime() ) );
        if( EventReceiver::getKey( PHK_UP ) ) system.getView().setPosition( system.getView().getPosition() + ( Vector2d( 0.0f , -100.0f ) * timer.getTime() ) );
        if( EventReceiver::getKey( PHK_DOWN ) ) system.getView().setPosition( system.getView().getPosition() + ( Vector2d( 0.0f , 100.0f ) * timer.getTime() ) );
